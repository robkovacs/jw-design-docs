function setLeftShadowPosition() {
    let leftShadows = document.querySelectorAll(
        ".table__container .table__scroll-shadow--left",
    );

    leftShadows.forEach((shadow) => {
        let widthBasis =
            shadow.nextElementSibling.querySelector("th:first-child");
        ...
        if (e.target.scrollLeft > parseInt(shadowWidth, 10)) {
            ...
            if (
                ...
                e.target.classList.remove(
                    "table__container--scrolled-to-right",
                );
            }
